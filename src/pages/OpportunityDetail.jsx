...
export default function OpportunityDetail({ opp, isBookmarked, onBookmark, onBack }) {
  // يجيب البيانات الكاملة من detail endpoint
  const [fullOpp, setFullOpp] = useState(opp);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`${BASE_URL}/opportunities/${opp.id}/`)
      .then(r => r.json())
      .then(data => {
        setFullOpp(data);
        setLoading(false);
  ...
  const deadlineStr = new Date(fullOpp.application_deadline).toLocaleDateString("en-US", {
    month: "long", day: "numeric", year: "numeric"
  });
  const colors = TYPE_COLORS[fullOpp.opportunity_type] || { bg: "#f0f0f0", text: "#666" };
  ...
  return (
    <div style={{
      ...
      <div style={{ background: "#e8e4f5", flex: 1, padding: "24px" }}>

        {loading ? (
          <div style={{ textAlign: "center", padding: "60px", fontSize: "18px", color: "#6b5c9e" }}>
            ⏳ Loading...
          </div>
        ) : (
          <div style={{
            display: "flex", gap: "24px",
            alignItems: "stretch", width: "100%", flexWrap: "wrap",
          }}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                {[
                  { icon: "📍", label: "Location", value: fullOpp.location },
                  { icon: "📅", label: "Deadline", value: deadlineStr },
                  { icon: "🕐", label: "Type", value: fullOpp.opportunity_type },
                  { icon: "📋", label: "Major", value: fullOpp.major },
                ].map((item, i) => (
                  <div key={i} style={{
              ...
              <p style={{ fontSize: "16px", fontWeight: 400, color: "#5E6670", lineHeight: "24px", marginBottom: "22px", fontFamily: "Inter, sans-serif" }}>
                {fullOpp.description || "No description available."}
              </p>

              {fullOpp.responsibilities && (
                <>
                  ...
                  <ul style={{ margin: "0 0 22px", paddingLeft: "20px", fontSize: "16px", fontWeight: 400, color: "#5E6670", lineHeight: "24px", fontFamily: "Inter, sans-serif" }}>
                    {fullOpp.responsibilities.split("\n").map((r, i) => (
                      <li key={i}>{r}</li>
                    ))}
                  </ul>
                </>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                  ...
                  <button onClick={() => onBookmark(fullOpp.id)} style={{
                    background: "none", border: "none", cursor: "pointer",
                ...
                {(fullOpp.is_expired || fullOpp.status === "expired") && (
                  <div style={{
                ...
                <div style={{ marginTop: "auto", display: "flex", flexDirection: "column", gap: "12px" }}>
                  <a
                    href={fullOpp.external_url}
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{
                      display: "flex", alignItems: "center", justifyContent: "center", gap: "8px",
                      background: fullOpp.is_expired ? "#aaa" : "#09ADB0",
                      color: "#fff", borderRadius: "8px",
                      padding: "0", fontSize: "14px", fontWeight: 600,
                      textDecoration: "none", transition: "background 0.15s",
                      width: "100%", height: "64px",
                      fontFamily: "Inter, sans-serif",
                      pointerEvents: fullOpp.is_expired ? "none" : "auto",
                    }}
                    onMouseEnter={e => { if (!fullOpp.is_expired) e.currentTarget.style.background = "#078a8c"; }}
                    onMouseLeave={e => { if (!fullOpp.is_expired) e.currentTarget.style.background = "#09ADB0"; }}
                  >
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "12px", height: "48px" }}>
                  <CompanyLogo company={fullOpp.organization_name} size={40} />
